        ...
        <div class="bg-surfaceDark border border-surfaceBorder rounded-xl p-5 space-y-4 shadow-xl">
          <div class="flex items-center justify-between border-b border-surfaceBorder pb-3">
            <div class="flex items-center gap-3">
              ...
              <div>
                <h3 class="font-bold text-sm text-slate-900 dark:text-white">Google Gemini AI Engine</h3>
                <p class="text-xs text-zinc-500 dark:text-zinc-400">Powers Lens AI Assistant, automated root-cause diagnostics, and trace queries (<span class="font-mono text-zinc-400">VITE_GEMINI_API_KEY</span>)</p>
              </div>
            </div>
            <span id="settings-gemini-status" class="px-2.5 py-0.5 rounded-full text-[10px] font-mono ${(window.GeminiService && window.GeminiService.isConfigured()) ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' : 'bg-amber-500/10 text-amber-400 border border-amber-500/20'}">
              ${(window.GeminiService && window.GeminiService.isConfigured()) ? '● GEMINI CONFIGURED' : '○ KEY NOT CONFIGURED'}
            </span>
          </div>
          ...
          <div class="space-y-4 text-xs">
            <div class="space-y-1.5">
              <div class="flex items-center justify-between">
                <label class="text-slate-900 dark:text-zinc-200">Google Gemini API Key *</label>
                <a href="https://aistudio.google.com/app/apikey" target="_blank" class="text-blue-400 hover:underline flex items-center gap-1 font-mono text-[11px]">
                  Get Free API Key from Google AI Studio <i data-lucide="external-link" class="w-3 h-3"></i>
                </a>
              </div>
              <div class="relative">
                <input 
                  ...
                  type="password" 
                  value="${(window.GeminiService && window.GeminiService.getApiKey()) || ''}" 
                  placeholder="AIzaSy..."
                  class="w-full px-3.5 py-2.5 pr-10 rounded-xl bg-surfaceElevated border border-surfaceBorder focus:border-brandBlue focus:ring-1 focus:ring-brandBlue text-slate-900 dark:text-white font-mono text-xs outline-none transition-all"
                />
                <button type="button" onclick="const input = document.getElementById('settings-gemini-key'); input.type = input.type === 'password' ? 'text' : 'password';" class="absolute right-3 top-2.5 text-zinc-400 hover:text-white cursor-pointer">
                  <i data-lucide="eye" class="w-4 h-4"></i>
                </button>
              </div>
              <p class="text-[11px] text-zinc-400 font-sans">Keys are saved locally in your protected <span class="font-mono text-blue-400">.env</span> file and never exposed publicly.</p>
            </div>

            <!-- Model Badge & Feature Capabilities -->
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-2.5 text-[11px] font-mono">
              <div class="p-2.5 rounded-lg bg-surfaceElevated border border-surfaceBorder space-y-0.5">
                <div class="text-zinc-400">Model Engine</div>
                <div class="text-white font-sans">Gemini 1.5 Flash</div>
              </div>
              <div class="p-2.5 rounded-lg bg-surfaceElevated border border-surfaceBorder space-y-0.5">
                <div class="text-zinc-400">Capabilities</div>
                <div class="text-white font-sans">Trace & Latency Reasoning</div>
              </div>
              <div class="p-2.5 rounded-lg bg-surfaceElevated border border-surfaceBorder space-y-0.5">
                <div class="text-zinc-400">Context Window</div>
                <div class="text-white font-sans">1 Million Tokens</div>
              </div>
            </div>
            ...
            <div class="flex items-center justify-between pt-2">
              <button 
                ...
                onclick="
                  ...
                  window.GeminiService.testConnection(key).then(res => {
                    btn.innerText = 'Test Connection';
                    if (res.success) {
                      window.AgentLensApp.showToast('Google Gemini connection verified successfully!', 'success');
                    } else {
                      alert('Gemini connection failed: ' + res.error);
                    }
                  });
                " 
                class="px-4 py-2 rounded-xl bg-surfaceElevated border border-surfaceBorder hover:border-brandBlue text-slate-900 dark:text-white text-xs flex items-center gap-2 transition-all cursor-pointer"
              >
              ...
              <button 
                onclick="
                  ...
                  const status = document.getElementById('settings-gemini-status');
                  if (status) {
                    status.className = 'px-2.5 py-0.5 rounded-full text-[10px] font-mono ' + (key ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' : 'bg-amber-500/10 text-amber-400 border border-amber-500/20');
                    status.innerText = key ? '● GEMINI CONFIGURED' : '○ KEY NOT CONFIGURED';
                  }
                  window.AgentLensApp.showToast('Gemini Configuration Saved & Activated!', 'success');
                " 
                class="px-4 py-2 rounded-xl bg-blue-600 hover:bg-blue-500 text-white text-xs shadow-lg shadow-blue-500/20 transition-all flex items-center gap-1.5 cursor-pointer"
              >
                <i data-lucide="save" class="w-3.5 h-3.5"></i>
